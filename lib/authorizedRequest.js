...
module.exports.generateAuthorizedRequestFromPhone = async function (phone, rawApplication) {
  ...
  const isFirstTime = !(await phoneTokenService.doesTokenExistForPhone(phone));
  const userToken = await phoneTokenService.getTokenFromPhone(phone);
  let arid = await generateAuthorizedRequest(userToken, rawApplication, isFirstTime);
  return arid;
}

async function generateAuthorizedRequest(userToken, rawApplication, isFirstTime) {
    ...
    let arid = await generateUniqueShortRandomString(
      Number(config.ARID_LENGTH),
      config.AUTHREQ_S3_BUCKET,
      s3keyPrefix);
